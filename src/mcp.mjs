...
export function createMcp({ baseUrl = process.env.ROBO_URL || 'http://127.0.0.1:4318', dataDir = process.env.ROBO_DATA_DIR || resolve(dirname(fileURLToPath(import.meta.url)), '../data'), fetchImpl = fetch } = {}) {
  ...
  const request = async (path, payload, signal) => {
    ...
    // P7: a command answers with what changed, not the whole state (60-75 KB in test 6); status keeps the full view.
    if (path === '/api/command') {
      const presenter = value.presenter ? { status: value.presenter.status, beat: value.presenter.beat, total: value.presenter.total, reason: value.presenter.reason } : null;
      const slide = value.slides?.[value.slideIndex];
      const summary = { ok: true, meeting: { status: value.meeting?.status, admitted: value.meeting?.admitted, sharing: value.meeting?.sharing }, voice: { desired: value.voice?.desired, status: value.voice?.status }, mode: value.mode,
        context: String(value.context ?? '').slice(0, 300), title: value.title, slides: value.slides?.length ?? 0, slideIndex: value.slideIndex, viewIndex: value.viewIndex ?? 0, views: slide?.views?.length ?? 0,
        highlight: slide?.views?.[value.viewIndex ?? 0]?.highlight ?? null, presenter, pendingJobs: (value.jobs || []).filter(job => job.status === 'pending').length, lastNote: value.notes?.at(-1)?.text?.slice(0, 200) ?? null };
  ...
  // P7: views (screen-sized windows of an image slide, page-normalized) were dropped by the schema before.
  const VIEW = z.object({ x: z.number().min(0).max(1), y: z.number().min(0).max(1), w: z.number().min(0).max(1), h: z.number().min(0).max(1), asset: z.string().max(500).optional(), say: z.string().max(900).optional(), lines: z.array(z.string().max(240)).max(60).optional(), highlight: z.object({ x: z.number(), y: z.number(), w: z.number(), h: z.number() }).optional() });
  tool('present', 'Display slides on the robot. A slide body is text, or image:/slides/<file>.png for a picture; an image slide may list views (screen-sized windows, page-normalized x, y, w, h, with an optional exact-render asset, say, lines and highlight) that stage/narrate move between. Optionally request the browser screenshare using enabled.', { title: z.string().max(300).default(''), slides: z.array(z.object({ title: z.string().max(300), body: z.string().max(5000), views: z.array(VIEW).max(60).optional() })).max(400), enabled: z.boolean().optional() }, args => request('/api/command', { type: 'present', ...args }));
  ...
  // P3: a pointer on the view that is on screen; cleared by off or the next move.
  tool('highlight', 'Point at something on the shared screen without moving it: draw a box around a phrase that is on screen, an equation row by its number, or a page-normalized rectangle. off removes it; the next move also removes it. Needs a deck built by present_pdf/present_file/present_deck for phrases and equations.', { phrase: z.string().min(1).max(300).optional(), equation: z.string().min(1).max(30).optional(), x: z.number().min(0).max(1).optional(), y: z.number().min(0).max(1).optional(), w: z.number().min(0).max(1).optional(), h: z.number().min(0).max(1).optional(), off: z.boolean().optional() }, args => request('/api/command', { type: 'highlight', ...args }));
