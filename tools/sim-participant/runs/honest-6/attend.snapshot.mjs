...
import { readyToGreet, cueFor } from '../src/greet-policy.mjs';
...
async function recapOfThisMeeting() {
  try {
    ...
    const lines = events.slice(starts[0].index).filter(event => event.type === 'transcript' && event.data?.text).map(event => `${event.data.role === 'assistant' ? 'You' : 'Someone'}: ${String(event.data.text).trim()}`);
...
function buildBriefing(recap) {
  return [
    ...
    `Your shared screen ${sharesAtJoin ? 'is on from the start' : 'starts off'} and turns on when a deck is presented (your present_slides tool, or the coding session presenting a PDF, a slide deck, a document, a web page or a picture; a document scrolls one screen-sized part at a time). You move it yourself with scroll (next, back, the top, the end, or a part number). Only the coding session can stop it or run a narrated presentation, in which RoboMeet shows each part and asks you to present it.`,
    // 2026-09-22 honest-tester runs: the robot read its screen notes aloud, talked through "just listen", and claimed moves,
    // zooms and coding-session replies that no tool made. One sentence each, stated as facts about how it works.
    'Notes about your screen (starting "SCREEN:", "On your shared screen now" or "About the box") are for you: never read them out or narrate them unasked.',
    'Only your tools act: never say you moved, boxed, saved, zoomed, posted, asked the coding session or put something up unless that tool ran and returned. If you have no tool for it, say so.',
    'Asked to just listen, answer in at most three words, then stay silent (no "mm-hmm", no reply to a rhetorical "right?") until someone addresses you by name.',
    ...
    'People besides Vivek may speak: use a name only when someone gave it or your purpose names them; never guess who is speaking.',
...
let aloneSince = 0, greeted = false, joinStartUsed = false, greetedBefore = false, leftAlone = false, rejoined = false; // src/greet-policy.mjs cueFor
...
async function policy({ meeting, voice }) {
  ...
  if (voice.status === 'idle' && voice.desired !== 'started') {
    ...
    if (!greeted && !options['no-greet'] && meeting.status === 'joined' && (humans === null || humans >= 1) && readyToGreet({ media, now })) { // src/greet-policy.mjs: not over someone talking
      greeted = true;
      const cue = cueFor({ greetedBefore, leftAlone, rejoined, greeting: greetingCue });
      greetedBefore = true; leftAlone = false;
      await command({ type: 'announce', text: cue, exact: false }).then(() => log('greeted', { humans, cue: cue === greetingCue ? 'greeting' : 'restart/rejoin' })).catch(error => log('greet_failed', { message: error.message }));
    }
    if (humans === 0) { greeted = false; leftAlone = true; }
...
async function main() {
  ...
  const recapAtLaunch = await recapOfThisMeeting();
  rejoined = Boolean(recapAtLaunch);
  let briefing = buildBriefing(recapAtLaunch);
